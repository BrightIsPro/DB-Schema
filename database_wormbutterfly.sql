...
-- Dumping data for table project_wormbutterfly.streaming_room: ~0 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  KEY `streaming_id0` (`streaming_id`),
  KEY `party_id0` (`party_id`),
  CONSTRAINT `party_id0` FOREIGN KEY (`party_id`) REFERENCES `party` (`party_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `streaming_id0` FOREIGN KEY (`streaming_id`) REFERENCES `streaming_room` (`streaming_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=2147483648 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Dumping data for table project_wormbutterfly.users: ~0 rows (approximately)
...
-- Dumping data for table project_wormbutterfly.users_streaming_room: ~0 rows (approximately)
